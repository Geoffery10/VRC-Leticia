...
float3 GetOutline(float3 col, float3 diff, int olMode){
    UNITY_BRANCH
    switch (olMode){
        case 1: diff = col; break;
        case 2: diff *= col; break;
        default: break;
    }
    UNITY_BRANCH
    switch (_FilterModel){
        case 1: diff = GetRGBFilter(diff); break;
        case 2: diff = GetHSLFilter(diff); break;
        default: break;
    }
    return diff;
}
...
float3 GetToonSpec(float3 diff, float3 hv, float3 nd, float3 lc){
    float brightness = GetWorldBrightness(lc);
    float3 sc = lerp(diff, _SpecCol.rgb, _SpecBias);
    float power = exp2(_SpecSize * 20.0 + 1.0);
