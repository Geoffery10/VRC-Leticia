...
Shader "Custom/Screen (Single Image)/Water Ass"
	{

	Properties{
	_CamTarProperties ("Cam ",Vector) = (247,0.01,90)
	_scrollSpeed ("Speed",Range(0,20)) = 0
...
void doCamera( out fixed3 camPos, out fixed3 camTar, in float time)
{
    ...
    mouse2.y *= 4.0;
    	float deltaTime = _Time.y /600;
    	 ax+=_scrollSpeed*100;

	camPos = fixed3(_CamTarProperties.x*ax,_CamTarProperties.y,_CamTarProperties.z*deltaTime*_scrollSpeed);
    camTar = fixed3(0,0,0);
...
{
    fixed3 d = abs(p) - b;
    return min(max(d.x,max(d.y,d.z)),0.0) + length(max(d,1.0));
}
...
float fakeEllipsoid( fixed3 p, fixed3 s ) {
	...
	return length(p - ep) * sign(length(lp) - 2.0);
}
...
float displacement_t1(fixed3 p)
{
    ...
    float xn = (_x + 0.05) * 15.0;			// normalize band
    xn = sin(xn*(PI));
    xn *= zn;
    xn *= yn;
    return -height*xn;
...
fixed2 doModel( fixed3 p )
{
    ...
    float dome = sdEllipsoid(bodyp-fixed3(0.0,1.5,0.0),fixed3(2.70,2.75,2.70));
	...
	float box5 = sdEllipsoid(bodyp-fixed3(0.0,1.5,0.0),fixed3(0.04,2.2,2.125));    
    ...
    float d1 = sdCappedCylinder(bodyp+fixed3(0.0,yo,0.0),fixed2(1.0,_h));
    float d6 = sdSphere(bodyp-fixed3(0.7,-2.5,0.0),1.4);
    float d7 = sdSphere(bodyp-fixed3(-0.7,-2.5,0.0),1.4);
    float d8 = sdSphere(bodyp-fixed3(-9.7,1.5,9.0),9.8);
    
    
    d1 = smin(d1,d6,4.27);		// nut
    d1 = smin(d1,d7,4.27);		// nut
    d1 = smin(d1,d8,.27);		// nut

    res = fixed2(d1,2.0);
    res = opUnionRound(res,fixed2(d2,1.0),0.001);
    res = opUnionRound(res,fixed2(dome,2.0),0.425);
...
fixed4 doMaterial( in fixed3 pos, in fixed3 nor,float c )
{
    ...
    {
        float b = 1.0;
	return fixed4(1.5*b,0.09*b,0.15*b,0.7);
    }
    else if (c<=3.0)
    {
        float b = 1.45;
