    ...
    float4 frag(v2f i, float facing: VFACE): SV_Target
    {
        ...
        float4 LightingRamp = tex2D(_Ramp, float2(FakeLight, FakeLight));
            ...
            {
                _flat_lighting_var = max(clamp(ambient + ambient * lerp(1, LightingRamp, _ShadowStrength) - ambient * (_ShadowStrength * lerp(.6, 1, _ForceShadowStrength)), 0, ambient), _MinBrightness);
            }
