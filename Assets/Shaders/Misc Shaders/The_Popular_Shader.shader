...
Shader "Custom/Screen (Single Image)/Disco Shader"
	{
...
static const float wlen=15.0;
float bob;
static const float wc_scale=0.5;
float scroll;
static const float scene_scale=15.0;
...
vec2 solve( vec2 p, float l1, float l2, float side )
{
	...
	if( s<0.0 ) return vec2(-100.0,-100.0);
...
float capsuleDist(vec3 p,vec3 o,vec3 d,float h0,float h1,float r0,float r1)
{
	...
	vec3 np=o+t*d;
	return distance(np,p)-mix(r0,r1,t)+mosaic(mul(m,(p-o)));
}

float boxDist(vec3 p,vec3 s,float r)
{
	return length(max(vec3(0.0,0,0),abs(p)-s))-r+mosaic(p);
}
...
float sceneDist(vec3 p)
{
	...
	// Arm 0
	{
		...
		float d0=capsuleDist(p+o,vec3(arm0[0],0.0),vec3(arm0[1]-arm0[0],0.0),0.0,0.7-g,0.03,0.03);
		float d1=capsuleDist(p+o,vec3(arm0[0],0.0)+vec3(arm0[1]-arm0[0],0.0),vec3(arm0[2]-arm0[1],0.0),g,0.7,0.03,0.06);
		d=min(d,smin(d0,d1,15.0));
	}
	...
	// Arm 1
	{
		...
		float d0=capsuleDist(p+o,vec3(arm1[0],0.0),vec3(arm1[1]-arm1[0],0.0),0.0,0.7-g,0.03,0.03);
		float d1=capsuleDist(p+o,vec3(arm1[0],0.0)+vec3(arm1[1]-arm1[0],0.0),vec3(arm1[2]-arm1[1],0.0),g,0.7,0.03,0.06);
		d=min(d,smin(d0,d1,15.0));
	}
...
{	
	vec2 slabs0 = (vec2(+1.0,+1.0) - ro) / rd;	
	vec2 slabs1 = (vec2(-1.0,-1.0) - ro) / rd;
...
vec3 squaresColours(vec2 p)
{
	...
	vec3 c=pow(l*(mix(orange,purple,0.5+0.5*cos(p.x*40.0+sin(p.y*10.0)*3.0))+
				  mix(orange,purple,0.5+0.5*cos(p.x*20.0+sin(p.y*3.0)*3.0))),vec3(1.2,1.2,1.2))*0.7;
...
vec3 squaresTex(vec2 p,float border)
{
	...
	m+=1.0-smoothstep(0.0,0.56,distance(fp,vec2(0.5,0.5)));
...
vec3 room(vec3 ro,vec3 rd,out vec3 rp,out vec3 n)
{
	...
	vec2 cp=vec2(0.0,0),ct=vec2(1e3,1e3);
	...
	vec3 cc=squaresTex(cu,0.45)+0.8*vec3(smoothstep(0.83/5.0,0.86/5.0,abs(rp.y)),smoothstep(0.83/5.0,0.86/5.0,abs(rp.y))
	,smoothstep(0.83/5.0,0.86/5.0,abs(rp.y)));
...
vec3 scene(vec2 p)
{
	mat3 cam = rotateXMat(cos(time * 0.0) * 0.0) * rotateYMat(time * 0.0);
	float lt=mod(time*wc_scale,wlen)/wlen;
	
	vec3 ro = mul(cam,vec3(0.0,-0.0+lt*0.0, 0.0+lt*0.0)+vec3(0.0,0.0,scroll/scene_scale));
	vec3 rd = mul(cam,vec3(p, -1.0));
	...
	if(robot_t < distance(ro,rp))
	{
		...
		n=sceneNorm(rp);
		vec3 r=reflect(rd,n);
		c=vec3(0.5+0.5*n.y,0.5+0.5*n.y,0.5+0.5*n.y)*0.5*vec3(1.0,0.8,0.5);
	...
	vec3 ll=vec3(1.0-(smoothstep(0.0,0.07,lt)-smoothstep(0.93,1.0,lt)),1.0-(smoothstep(0.0,0.07,lt)-smoothstep(0.93,1.0,lt))
	,1.0-(smoothstep(0.0,0.07,lt)-smoothstep(0.93,1.0,lt)));
	
	return ll+c+
		0.6*((sin(p.y)*cos(p.x+time*2.0)*0.5+0.5)*
			 pow(mix(vec3(1.0,0.7,0.1),vec3(1.0,0.2,0.6),0.5+0.5*cos(p.x+sin(time*3.0+p.y*2.0))),vec3(2.0,2.0,2.0)));
}
	...
	time=_Time.w+1.0;
	bob=cos(time*0.0)*0.00;
	scroll=-15.0+mod(time*wc_scale,wlen)*5.0;
	vec2 uv = 1.0;
	vec2 q=uv;
	vec2 t=normalize(i.worldSpaceView);
	t.x*=1/1;
