    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        // Scale
        float2 st = frac(poiMesh.uv[0] + float4(0.1,0,0,0).xy * _Time.x) * float(309.39);
        ...
        switch(float(0))
        {
            ...
            if (float(0) == 1 || float(1424.7) != 0)
            {
                ...
                float theta = radians((randomBoy + _Time.x * float(1424.7)) * 360);
                float cs = cos(theta);
                float sn = sin(theta);
        ...
        switch(float(0))
        {
            ...
            finalGlitter = lerp(0, float(0) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.8)), poiCam.viewDir)), float(353)), 0);
        ...
        if(float(0) == 1 || float(1424.7) != 0 && !float(0))
        {
        ...
        if(float(0) == 1)
        {
            albedo.rgb = lerp(albedo.rgb, finalGlitter * glitterColor * float(4.9), finalGlitter * glitterTexture.a * glitterMask);
            glitterEmission = finalGlitter * glitterColor * max(0, (float(4.9) - 1) * glitterTexture.a) * glitterMask;
        }
        else
        {
            glitterEmission = finalGlitter * glitterColor * float(4.9) * glitterTexture.a * glitterMask;
        }
    }
